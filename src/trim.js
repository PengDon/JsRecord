 ...
 * @param {String} str 
 */
const trim = (model, str) => {
    let newStr = '';
    switch (model) {
        case 1:
            // 去除两边空格，使用两次正则替换，实际速度也非常快.目前chrome对字符串方法进行了疯狂的优化，引起了其他浏览器的跟风，于是正则的实现再也比不了字符串方法了
            newStr = str.replace(/^\s\s*/, '').replace(/\s\s*$/, '');
            break;
        case 2:
            // 写法2与写法1很相似，但稍慢一点，主要原因是它最先假设至少存在一个空白符。Prototype.js使用的这种实现。不过其名字为strip，因为Prototype的方法都是力求与Ruby同名
            newStr = str.replace(/^\s+/, '').replace(/\s\s+$/, '');
            break;
        case 3:
            // 方法3通过截取方式取得空白部分（并且容许中间存在空白符），总共调用了4个原生方法。设计非常巧妙，substring以两个数字作为参数。Math.max以两个数字作为参数，search返回一个数字。速度上比上面两个方法慢一点，但比方法4-9要快。
            newStr = str.substring(Math.max(str.search(/\S/), 0), str.search(/\S\s*$/) + 1);
            break;
        case 4:
        ...
        case 6:
            ...
            newStr = str.replace(/^\s*(\S*(\s+\S+)*)\s*$/, '$1');
            break;
        case 7:
        ...
        case 9:
            ...
            newStr = str.replace(/^\s*([\S\s]*?)\s*$/, '$1');
            break;
        case 10:
            // 先把可能的空白符全部列出来，在第一次遍历中砍掉前面的空白，第二次砍掉后面的空白。全过程只用了indexOf与substring这个专门处理字符串而生的原生方法，
            // 没有使用到正则。速度快的惊人，估计直逼内部的二进制实现，并且在火狐与IE（其他浏览器当然也毫无疑问）都有良好的表现，速度都是零毫秒级别的，Php.js就收纳了这个方法。
            let whitespace = ' \n\r\t\f\x0b\xa0\u2000\u2001\u2002\u2003\n\\u2004\u2005\u2006\u2007\u2008\u2009\u200a\u200b\u2028\u2029\u3000';
            ...
            newStr = whitespace.indexOf(str.charAt(0)) === -1 ? str : '';
            break;
        case 11:
            ...
            break;
        case 12:
            // 方法12：针对方法10改进，改进的不是性能速度，而是易记与使用
            let m = str.length;
            for (let i = -1; str.charCodeAt(++i) <= 32;)
                for (let j = m - 1; j > i && str.charCodeAt(j) <= 32; j--)
